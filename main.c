...
#include <stdbool.h>
#include "d_led.h"			// Dołączenie biblioteki wyświetlacza led.

#define KEY_12H_24H 	(1<<PD2) 	// Pin dla klawisza odpowiedzialnego za przełączenie trybu godzinowego zegara 12/24H.
#define BUZZER 		(1<<PD3) 	// Pin dla brzęczka sygnalizującego.
#define SET_TIME 	(1<<PD4) 	// Pin dla klawisza odpowiedzialnego za uruchamianie trybu ustawiania godziny.
#define INCREASE_VALUE 	(1<<PD5) 	// Pin dla klawisza odpowiedzialnego za zwiększanie godziny/minut.
#define DECREASE_VALUE 	(1<<PD6) 	// Pin dla klawisza odpowiedzialnego za zmniejszanie godziny/minut.
#define SET_MM_HH 	(1<<PD7) 	// Pin dla klawisza odpowiedzialnego za przełączenie między ustawianiem godzin lub minut.
#define SIGNAL_LED	(1<<PC7) 	// Pin dla diody sygnalizującej wciśnięcie przycisku.

uint8_t key_down (uint8_t KEY);		// Funkcja obsługująca naciskanie klawiszy.
void set_time_function (void);		// Funkcja ustawiania godziny.
void time_form (void);			// Funkcja ustawiania formatu godziny.
void blink_digits (void);		// Funkcja odpowiedzialna za mruganie  całego wyświetlacza.
...
bool interval (uint16_t millis);	// Funkcja wystawiająca flagę co zadany w ms interwał.

uint8_t D,  mj, md, gj ,gd ; 	 	// m - minuty, g - godziny, j- cyfra jedności, d - cyfra dziesiątek
uint8_t b=0;				// Zmienne przełączająca stany.
static uint8_t s=0;
volatile static uint16_t i=0;		// Zmienna zegarowa
volatile static uint32_t _interval=0;
uint8_t hour_quantity;
uint16_t blink_ms=200; 			// Jak często  mrugac?
...
int main(void)
{
	...
	// Klawisze
	DDRD &=~(KEY_12H_24H | SET_TIME | INCREASE_VALUE | DECREASE_VALUE | SET_MM_HH); // Piny jako wejścia.
	PORTD|=  KEY_12H_24H | SET_TIME | INCREASE_VALUE | DECREASE_VALUE | SET_MM_HH; 	// Wymuszenie stanu wysokiego.

	// Sygnalizacja
	DDRC |= SIGNAL_LED;	// Pin jako wyjście.
	PORTC|= SIGNAL_LED;	// Stan wysoki, LED wyłączony.
	DDRD|=BUZZER; 		// Pin jako wyjście.
	PORTD&=~BUZZER;		// Stan wysoki, brzęczek wyłączony.

	d_led_init();  		// Inicjalizacja wyświetlacza multipleksowanego.
	sei();			// Włączenie globalnego zezwolenia na przerwania.

	hour_quantity=24;	// Ustawienia początkowe formatu godzinowego. Wartośc początkowa zegara, godzina wyświetlana po włączeniu zasilania.
	D=12;
	md=0;
	mj=0;
...
if(hour_quantity==24&&d==1)				// Wyświetlanie info o trybie godzinowym. 24H
	{
	cy1=2;
	cy2=4;
	cy3=11;
...
if(hour_quantity==12&&d==1)				// Wyświetlanie info o trybie godzinowym. 12H
	{
	cy1=1;
	cy2=2;
	cy3=11;
	...
	if ((hour_quantity==12)||(hour_quantity==24)) // Częśc wspólna dla obu trybów.
		{
			switch (D)
			{
	...
	if (hour_quantity==12)// tryb 12h			// Godziny popołudniowe i wieczonrne w trybie 12H.
		{
			switch (D)
			{
	...
	if (hour_quantity==24)//tryb 24h			// Godziny popołudniowe i wieczonrne w trybie 24H.
		{
			switch (D)
			{
